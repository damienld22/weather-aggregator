...
describe("MeteocielAdapter", () => {
  ...
  test("In case of HTML table with some rain, the proper rain values should be returned", async () => {
    ...
    mockScrapper.mockResolvedValue(document.querySelectorAll("tr"));
  ...
  test.fails(
    "In case of we are in the end of the month, the date should be the proper one",
    () => {}
  );
});
